...
def test_build_coverage_sql_shares_hex_counts_cte_by_default() -> None:
    ...
    assert sql.count("JOIN hex_counts AS hc") == 1
...
def test_build_coverage_sql_uses_provided_hex_counts_relation() -> None:
    ...
    assert sql.count("JOIN precomputed_counts AS hc") == 1
    assert "CASE" in sql
...
def test_build_coverage_sql_requires_containment_for_full_coverage() -> None:
    ...
    assert "1.0" in sql
    assert "ST_MakeValid(a.geom)" not in sql
    assert "ST_MakeValid(j.hex_geom)" in sql
    assert "ST_MakeValid(j.poly_geom)" in sql
